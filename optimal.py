...
# ax3.set_ylabel("Integrals (W/m^2)")
...
times = pd.date_range(
    start_date, end_date)
#print(times.date.tolist())
energy_production = []
for day in times.date.tolist():
    delta_time = "Min" 
    time = pd.date_range(
    start_date + " 00:00:00", end_date + " 23:59:00", inclusive="left", freq=delta_time)
    sunpos = site.get_solarposition(time)
    chosen_date = str(day)#.strftime("%Y-%m-%d")
    #print(chosen_date)
    daily_energy = integrate.simps(solar_flux(sunpos.loc[chosen_date].zenith, sunpos.loc[chosen_date].azimuth, theta_panel, phi_panel), np.arange(0, 24*60, 1), dx=60)
    print(daily_energy)
#     energy_production.append(daily_energy)
# print(energy_production)
# fig, ax = plt.subplots(figsize=(10, 5))
# ax.plot(times.date.tolist(), energy_production)
# ax.set_ylabel("Energy Production (W/m^2)")
# ax.set_xlabel("Date")
