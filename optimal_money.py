...
#end_date = "2024-12-31"
#time = np.arange(0, 365*24, 1) #time in minutes for a full year

# Dates that have a datapoint for price
start_date = "2020-01-01"
end_date = "2020-11-22"
time = np.arange(0, 326*24, 1) #time in minutes for period of price data
...
ax3.set_ylabel("Total kWh for given angle")
ax3.set_xlabel("Theta_p angle (degrees)")
ax3.set_title("Plot of total energy production in kWh per panel in period")

#plt.show()

max_prod=np.max(int_values)
optimal_angle=np.argmax(int_values)
print("The optimal angle for energy is: ", optimal_angle, "degrees")


#### Extension
with open('Spotprices by hour 01-01-2020 to 21-11-2020.csv', 'r') as file:
    data = np.genfromtxt(file, delimiter='\n', dtype=float)

# data only covers 01-01-2020 to 21-11-2020, so we need new start and end date
start_date = "2020-01-01"
end_date = "2020-11-22"
times = pd.date_range(
    start_date + " 00:00", end_date + " 00:00", inclusive="left", freq=delta_time, tz=timezone)

# Getting solar position for new dates
sunpos = site.get_solarposition(times)

#new angles for new dates
theta_sun = np.array(sunpos.zenith)
phi_sun = np.array(sunpos.azimuth)

# time is different because it is not a full year
time = np.arange(0, 326*24, 1) #time in minutes for period of price data

# flux function but for money, instead of power
def money_flux(theta_sun, phi_sun, theta_panel, phi_panel):
    dot_products=solar_panel_projection(theta_sun, phi_sun, theta_panel, phi_panel)
    #flux_array=np.empty_like(dot_products)
    flux_array = np.array([])
    for element in dot_products:
    ...
    money_flux_array = np.array([])
    for n in range(len(flux_array)):
        if data[n] > 0:
            money_flux_array = np.append(money_flux_array, (data[n] * flux_array[n])/100)
        else:
            money_flux_array = np.append(money_flux_array, flux_array[n])
    return money_flux_array

int_values=np.array([])
for i in range(0, 90):
    theta_panel = i
    integral_value = integrate.simps(money_flux(theta_sun, phi_sun, theta_panel, phi_panel),time, dx=60)
    int_values = np.append(int_values, integral_value)

fig, ax4 = plt.subplots(figsize=(10, 5))
ax4.plot(angles, int_values)
ax4.set_ylabel("Total DKK for given angle")
ax4.set_xlabel("Theta_p angle (degrees)")
ax4.set_title("Plot of total earnings in DKK per panel in period")

max_prod=np.max(int_values)
optimal_angle=np.argmax(int_values)
print("The optimal angle for money is: ", optimal_angle, "degrees")
